...
class SwanlabWriter(EventWriter):
    """
    Write all events to a SwanLab experiment.

    Parameters (copied from swanlab official code)
    ----------
    ...
    def __init__(self, window_size: int = 20, **kwargs):
        self._window_size = window_size
        self._writer_args = kwargs
        self._last_write = -1
    
    @cached_property
    def _writer(self):
        import swanlab
        swanlab.init(**self._writer_args)
        return swanlab
    ...
    def write(self):
        ...
        if len(storage._objs) >= 1:
            for obj, iter in storage._objs:
                self._writer.log(obj, step=iter)
            storage.clear_objs()
...
class EventStorage:
    ...
    def put_obj_dict(self, obj_dict: dict, cur_iter=None):
        new_obj_dict = {}
        for k, v in obj_dict.items():
            name = self._current_prefix + k
            new_obj_dict[name] = v
        cur_iter = self._iter if cur_iter is None else cur_iter
        self._objs.append((new_obj_dict, cur_iter))
        
    def put_obj_dicts(self, objs: list):
        """
        Add a dictionary of objects to the storage.
        This will provide support for swanlab visualization.
        """
        for (obj, iter) in objs:
            assert type(obj) is dict, "Expected a dictionary object format"
            self.put_obj_dict(obj=obj, cur_iter=iter)
    ...
    def clear_objs(self):
        """
        Delete all the stored object for swanlab log.
        This should be called after object are send to swanlab.
        """
        self._objs = []
